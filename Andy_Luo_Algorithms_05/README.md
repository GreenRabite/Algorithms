...
Also, in the skeleton file for the `BinarySearchTree` class, there are method arguments that have a variable named `node`. A `node` is essentially just a root node, and we call it a `node` because it holds access to the rest of the tree. We didn't want to also call it `root_node` since we already had a `@root`.
...
In the file, `bst_practical.rb`, write a function that takes in a binary search tree (as a `node`) and an integer *k*, and returns the <i>k</i>th largest element in the BST.
